...
#'
#'              Because this function is part of the \code{MLSurvey} package,
#'              each worker simply attaches \code{MLSurvey} (along with
#'              \code{glmnet} and \code{survey}) via \code{library()}, which
#'              makes [wElnet()], [replicate_weights()], and all of their
#'              internal helper functions available on every worker
#'              automatically -- no manual \code{clusterExport()} of function
...
#'                      allowed to use. Defaults to \code{1}, to avoid
#'                      oversubscribing CPU cores when \code{n.cores} worker
#'                      processes each also try to run multithreaded linear
#'                      algebra inside \code{glmnet}. Requires the
#'                      \code{RhpcBLASctl} package; if it is not installed,
#'                      this is silently skipped with a message.
...
#'
#' @return An object of class \code{par.w.elnet}, a list with:
...
#'
#' en.par$summary      # error per alpha
#' en.par$best.alpha    # winning alpha
...
par_wElnet <- function(alpha = c(0.000, 0.001, 0.008, 0.027, 0.064, 0.125, 0.216,
  ...
  # Attach MLSurvey (which brings wElnet(), replicate_weights(), and all of
  # their internal helpers along with it), plus glmnet and survey, on every
  # worker. Since this function now lives inside the MLSurvey package itself,
  # there is no need to clusterExport() any function definitions by name --
  # library(MLSurvey) makes the whole package namespace available on each
  ...
  fit_one_alpha <- function(a){
    ...
    do.call(MLSurvey::wElnet, call.args)
  }
