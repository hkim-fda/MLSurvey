...
#' @param nfolds An integer. The number of folds to be defined. Default is \code{nfolds=10}. Only applies to the \code{dCV} method.
#' @param R An integer. The number of times the sample is partitioned. Default is \code{R=1}. Only applies to \code{dCV}, \code{split} or \code{extrapolation} methods.
#' @param B An integer. The number of bootstrap re-samples. Default is \code{B=200}. Only applies to \code{bootstrap} and  \code{subbootstrap} methods.
#' @param train.prob A numeric between 0 and 1 that sets the proportion of clusters (for the method \code{split}) or strata (for the method \code{extrapolation}) in training sets. Default is \code{train.prob=0.7}. Only applies to \code{split} and \code{extrapolation} methods.
...
#' @param dCV.sw.test A logical value. If \code{TRUE}, original sampling weights for the units in the test sets are returned instead of the replicate weights. Default is \code{dCV.sw.test = FALSE}. Only applies to \code{dCV} method.
#'
...
#' The number of new columns and their corresponding names are usually determined by numeric arguments, i.e.,\code{nfolds}, or \code{R}, etc.
...
replicate_weights <- function(data,
  ...
  if(method == "dCV"){
    ...
    newdata <- cv_folds(data, nfolds, weights, strata, cluster, R, rw.test, dCV.sw.test)
...
# @export
cv_folds <- function(data, nfolds, weights, strata=NULL, cluster=NULL, R=1, rw.test, dCV.sw.test){
  # function for dCV
  ...
  for(r in 1:R){
    ...
    data[,paste0("folds_",r)] <- f.folds(data, nfolds=nfolds, strata=strata, cluster=cluster)
...
f.folds <- function(data, nfolds=5, strata=NULL, cluster=NULL){
  ...
  v.folds.newids <- sample(cut(seq(1,n),breaks=nfolds,labels=FALSE))
