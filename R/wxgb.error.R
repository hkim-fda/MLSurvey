...
#' @param dtrain An object of \code{xgboost::xgb.DMatrix} including `label`(response variable or binary outcome), `data`, and `weight`.
...
#' @seealso [optim_wxgb_para()] for detailed optimization and examples.
#'
#
#'
#'@export
evalerror_bin<-function(preds,dtrain){
  labels<-getinfo(dtrain,"label")
  weights <- getinfo(dtrain,"weight")
  err <- stats::weighted.mean(labels!=(preds>0),weights,na.rm=TRUE)

  return(list(metric="weighted.mean.error", value=err)) }
...
#'
#' @export
eval_loss<- function(preds,dtrain){
  labels<-getinfo(dtrain,"label")
  weights <- getinfo(dtrain,"weight")
  preds <- 1/(1+exp(-preds))
...
#' @param dtrain An object of \code{xgboost::xgb.DMatrix} including `label`(response variable), `data`, and `weight`.
#' @return A list of two components:
...
#'
#' @export
evalerror_reg <- function(preds,dtrain){
  labels <- getinfo(dtrain,"label")
  weights<- getinfo(dtrain,"weight")
