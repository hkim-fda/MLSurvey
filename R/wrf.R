...
#'
#'
#' @seealso \code{\link{[randomForest]{randomForest}}} for arguments and return values in detail.
#'
...
#'   - `all`: A numeric vector of `nfolds*R` `mtry`- values, i.e. `mtry` for each fold per replicate.
...
#' - `model`: A list containing information on the fitted model by \code{optimal.mtry}: an object of class \code{randomForest::randomForest}.
...
#'
#'@note
#' `final.model` can leverage R-\code{randomForest} functionality for comprehensive data analysis for linear/logistic regression.
#'
#' @examples
...
#'               method = "dCV", nfolds=5, R=5,importance=TRUE,proximity = TRUE)
#'
#' # Or equivalently:
...
wRandomforest <- function(data = NULL, col.x = NULL, y = NULL, xtest=NULL,ytest=NULL,
                   ...
                   nfolds = 10, R = 1, B = 200, dCV.sw.test = FALSE, train.prob = 0.7,
