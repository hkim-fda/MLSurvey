...
#'
#'  data(nhanes2013_sbc)
#' # The following sequence of default alpha's is generated, 
#' # and one can choose one or more for model fitting.
...
################ for each alpha
 modlist<- if(!is.null(ncores)){
              ...
              foreach::foreach(a=alpha, .packages = `glmnet`)%dopar%{
                .wcv_alpha(a,newdata,col.y,col.x,lambda=lambda,weights=weights,R=R,nfolds=nfolds,family=family,...)
              }
                
              doParallel::stopImplicitCluster()   
             } else {
               lapply(alpha, .wcv_alpha,newdata=newdata,col.y=col.y,col.x=col.x,lambda=lambda,weights=weights,R=R,nfolds=nfolds,family=family,...)
             }
...
.wcv_alpha<- function(alpha,newdata,col.y,col.x,lambda=NULL,weights=NULL,R,nfolds,family,...){
