...
#'
#'@description A function fitting Elastic Net prediction (linear or logistic) models with sampling weights in the estimation process
#'            to select a lambda minimizing the weighted mean error for a selected replicate weights method.  This is extended from
#'             `wlasso::wlasso()`, which is now officially available in \code{\link[svyVarSel]{wlasso}}.  This function is currently run with a single value of ``alpha`` but will
#'             support for the operation with a sequence of ``alpha`` soon.
#'
